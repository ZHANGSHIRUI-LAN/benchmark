...
void GpuResource::CudaMemEstabulished(int row,int col,int gpu_number,TGPUplan * plan)
{
	for (int  i = 0; i <gpu_number; i++)
	{
		...
		plan[i].h_matrix_b_tran = new Matrix<float>(col, row, MemType::HostMem);
		plan[i].h_matrix_b_tran->ResetShape(col, row);
		...
		plan[i].d_matrix_c_trans = new Matrix<float>(col, row, MemType::DeviceMem);

...
public:
  ...
  //矩阵点乘
  void CudaMatrixDotproduct(float *matrix_A, float *matrix_B, float *matrix_C, int row, int col, cudaStream_t st);

  // 矩阵乘法
  void CudaMatrixMul(float *matrix_A, float *matrix_B, float *matrix_C, int a_row, int a_col, int b_row, int b_col, float alpha, float beta, cublasHandle_t handle);

  //矩阵除法
  void CudaMatrixDivision(float *matrix_A, float *matrix_B, float *matrix_C, int row, int col, cudaStream_t st);

  //矩阵减法
  void CudaMatrixSubtraction(float *matrix_A, float *matrix_B, float *matrix_C, int row, int col, cudaStream_t st);
